...
#    (`dist/` + `styles/` + 运行期清单),没有库源码.
# 3. **产物是只读的**:本站一行都不改它,也不在应用侧给库的节点补类名 / 改结构去
#    迁就它.**一切以库为准**.确实要改就改库仓库(GitHub 上的 `miko_ui`,或本机的
#    相邻工作副本 `../miko_graphcalc/packages/miko_ui`),重新出一次资产再由这里同步.
#    只想按主题换口径时,覆盖只写在应用自己的样式表里
#    (`src/metro_window/src/metro_window.css`).
# 4. 产物的**运行期依赖由应用侧声明**.根 package.json 必须自己写
#    `"@preact/signals-core"`(库的运行时依赖;引公式时还有 `katex`):
#    `file:` 链接的传递依赖装不装,取决于目标目录在不在应用根内(npm 实测:目标在
#    根内会装,在根外不装) -- 把它写成应用自己的依赖,行为才不依赖 npm 这个细节,
#    也才能保证全程只有一份实例.应用自己声明之后,`vite.config.ts` 的
#    `resolve.dedupe: ['@preact/signals-core']` 是第二道保险.
...
#     产物里没有 `scripts/` 与 devDependencies,那一步必炸
...
掩盖成绿色).要靠本地源码构建来排查库的问题时,直接去库仓库工作副本里 \`npm run
build:dist\`,那是库自己的事,不经过本仓库.
GUIDE
}

# ---------- 产物校验 ----------
# 守三种静默错配:解开了空/半截 tarball,资产打的是源码而不是产物,资产清单里带着
# `scripts`(npm 装 `file:` 依赖时会去跑库的 build,产物里没有 scripts/ 必炸).
#
