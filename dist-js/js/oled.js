...
export class OLEDCanvas {
    constructor(config = {}) {
        ...
        // ======================
        // 画笔颜色
        this.pixelColorMode = 'dark';
        // 'lsb' | 'msb' (低位/高位模式)
        this.byteOrderMode = 'lsb';
        // 绘图工具设置
        this.currentTool = 'free';
        // 记录位置,首次/最后按下
        this.startPos = null;
        this.lastPos = null;
        // 存储预览前的画布
        ...
        if (!ctx)
            throw new Error('[OLEDCanvas] Canvas 2D 上下文不可用');
        this.ctx = ctx;
        // 设置物理像素尺寸(实际分辨率)
    ...
    */
    ...
    bindEvents() {
        ...
        document.querySelectorAll('input[name="tools"]').forEach(radio => {
            radio.addEventListener('change', () => this.setTool(radio.value));
        });
        // ======================
        // 鼠标事件监听
        // ======================
    ...
    async copyExport() {
        ...
        try {
            ...
            if (this.copyBtn) {
                ...
                setTimeout(() => {
                    if (this.copyBtn)
                        this.copyBtn.textContent = '复制到剪贴板';
                }, 4000);
            }
        }
        catch (err) {
     ...
     */
    walkBresenham(x1, y1, x2, y2, callback) {
    ...
    previewLine(endX, endY) {
        if (!this.previewImageData || !this.startPos)
            return;
        // 1. 恢复预览前状态
        this.ctx.putImageData(this.previewImageData, 0, 0);
        // 2. 创建临时canvas实现预览效果
        ...
        for (let page = 0; page < this.canvas.height / 8; page++) {
            ...
            for (let x = 0; x < this.canvas.width; x++) {
                ...
                for (let bit = 0; bit < 8; bit++) {
                    ...
                    // 判断像素颜色(黑色为1)
                    const isBlack = this.imageData.data[idx] === 0 &&
    ...
    getPixelPosition(event) {
        return {
            x: Math.min(this.canvas.width - 1, Math.max(0, Math.floor(((event.clientX - this.canvasRect.left) / this.canvasRect.width) * this.canvas.width))),
            y: Math.min(this.canvas.height - 1, Math.max(0, Math.floor(((event.clientY - this.canvasRect.top) / this.canvasRect.height) * this.canvas.height))),
        };
    }
    // 更新指示器位置
    updateIndicator(pos) {
