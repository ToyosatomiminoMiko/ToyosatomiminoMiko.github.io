...
`#metro-window` 空宿主上(挂载见 `src/main.ts`,源码在 `metro_window/`).
搬入前它是独立仓库
...
| 地铁车窗前端 | `metro_window/web/src/config.ts`,`metro_window/web/src/ui/`,`metro_window/web/src/tokens.css` | DOM id / class,`data-*` 键名,`setParam` 参数名映射,车窗标记与设置面板的声明式模型(标题/副标题/画布分辨率/滑块分组 / 风格 / 按钮 / 文案);声明式 DOM 组件(`h()` + 车窗标记 + 设置面板);组件设计令牌 |
