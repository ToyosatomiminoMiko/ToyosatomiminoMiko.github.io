...
class MathEvaluator {
     ...
     *
     * @param {function(number,number):number} fn   - 二元函数
     * @param {number} xMin - x 范围下界
     * @param {number} xMax - x 范围上界
     * @param {number} yMin - y 范围下界
     * @param {number} yMax - y 范围上界
     * @param {number} cols - 网格列数(x 方向分段数)
     * @param {number} rows - 网格行数(y 方向分段数)
     * @returns {Float32Array} 扁平数组 [x0,y0,z0, x1,y1,z1, ...]
     */
    static computeGrid(fn, xMin, xMax, yMin, yMax, cols, rows) {
     ...
     * @param {number} cols - x 方向分段数
     * @param {number} rows - y 方向分段数
     * @returns {number[]} 索引数组 [a1,b1,d1, a1,d1,c1, ...]
     */
    static generateIndices(cols, rows) {
        const indices = [];
        for (let j = 0; j < rows; j++) {
            for (let i = 0; i < cols; i++) {
                const a = j * (cols + 1) + i;
...
class SurfaceMesh {
    /**
     * @param {number} [cols=128] - x 方向网格分段数
     * @param {number} [rows=128] - y 方向网格分段数
     */
    constructor(cols = 128, rows = 128) {
        this.cols = cols;
        this.rows = rows;
...
export { MathEvaluator, SurfaceMesh };
